...
public class QueryExecutor {
    ...
    public void execute(String query) {
        if (isShowTablesCommand(query)) {
            ...
            DDL.createTable(query.substring(Command.CREATE_TABLE.length()));
        }
        else if (isDropTableCommand(query)) {
            DDL.dropTable(query.substring(Command.DROP_TABLE.length()));
        }
        else if (isInsertIntoCommand(query)) {
            DML.insertInto(query.substring(Command.INSERT_INTO.length()));
        }
        else if (isDeleteFromCommand(query)) {
            DML.deleteFrom(query.substring(Command.DELETE_FROM.length()));
        }                                
        else if (isUpdateCommand(query)) {
            DML.update(query.substring(Command.UPDATE.length()));
        }
        else if (isSelectCommand(query)) {
            VDL.select(query.substring(Command.SELECT.length()));
        }
